    ...
    def dataParse(self, data):
        ...
        try:
            ...
            if data_type == 1 : 
                ...
                self._setDataValue(str_temp, data[3] << 8 + data[4])
            elif data_type == 2 :
                for i in range(32) :
                    str_temp = str_name + str(i+1) + 'Error'
                    if i < 8:
                        self._setDataValue(str_temp,(data[3] & (1 << i)) >> i) 
                    elif i < 16:
                        self._setDataValue(str_temp,(data[4] & (1 << (i-8))) >> (i-8))
                    elif i < 24:
                        self._setDataValue(str_temp,(data[5] & (1 << (i-16))) >> (i-16))
                    else:
                        self._setDataValue(str_temp,(data[6] & (1 << (i-24))) >> (i-24))
            elif data_type == 4 :
                for i in range(4) :
                    str_temp = str_name + str(i+1)
                    if i ==  1 :
                        self._setDataValue(str_temp,(data[i*2+3] << 8) + data[i*2+4])
                    else :
                        if 0xff == data[i*2+3]:
                            self._setDataValue(str_temp,((data[i*2+3] << 8) + data[i*2+4] - 65536)//10)
                        else:
                            self._setDataValue(str_temp,((data[i*2+3] << 8) + data[i*2+4])//10)
            elif data_type == 5 :
                for i in range(5) :
                    str_temp = str_name + str(i+1)
                    self._setDataValue(str_temp,((data[i*2+3] << 8) + data[i*2+4])//10)
