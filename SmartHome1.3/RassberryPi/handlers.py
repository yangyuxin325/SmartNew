...
def handleAskInitData(area_connection):
    print "handleAskInitData"
#     同步数据

                    
def handleDataUnit(client):
    buf = client.recv(16)
    print buf.strip()
    if len(buf) == 16 :
        head = struct.unpack('!4i',buf)
        print head
        if head[3] > 0 :
            if client.addr[0] == '172.16.1.13' and head[2] == SmartServer().lastMsgNum:
#             初始化数据，开始巡检
                SmartServer().lastMsgNum = None
                SmartServer().allChannelStart()
            buf = client.recv(head[3])
            if len(buf) == head[3] :
                body = json.loads(buf)
                print body
                ...
                try:
                    ...
                    print "handleDataUnit got Error : ", e
                    
def handleDataArea(client):
    buf = client.recv(16)
    print buf.strip()
    if len(buf) == 16 :
        head = struct.unpack('!4i',buf)
        ...
        if head[3] > 0 :
            ...
            if len(buf) == head[3] :
                ...
                try:
                    ...
                    print "handleDataArea got Error : ", e
